---
name: openspec-decompose
description: Decompose a PRD into a Feature Spec with multiple changes and dependency tracking. Use when the user has a large requirement that needs to be broken down into multiple focused OpenSpec changes before implementation.
license: MIT
...
**Input**: The user's request should include a file path to a PRD document, a text description of the requirement, or nothing (will prompt for input).
